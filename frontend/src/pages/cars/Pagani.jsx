...
function Pagani() {
    const [pagani, setPagani] = useState([]);
    const [error, setError] = useState("");
    const [favorites, setFavorites] = useState({});
    const navigate = useNavigate();

    const fetchPagani = async () => {
        try {
            const response = await fetch("http://localhost:3000/Pagani");
            ...
            setError("Hiba a Pagani-k lekérésénél.");
        }
    };
    ...
    return (
        <div className="mt-5">
            <h2 className="text-center mb-5">Pagani</h2>
            ...
            >
                {pagani.length > 0 ? (
                    ...
                    <p className="text-center">A pillanatban nincsenek Pagani autóink.</p>
                ) : null}
            </Masonry>
        </div>
    );
